...
void main()
{
    ...
    vec4 mrSample = texture(gMetallicRoughness, vTexcoord);
    ...
    vec4 baseColor = SRGBtoLINEAR(texture(gAlbedoSpec, vTexcoord));
    vec3 f0 = vec3(0.04);
    ...
    vec3 n = texture(gNormal, vTexcoord).rgb;                             // normal at surface point
    
    // vec3 v = vec3( 0.0, 0.0, 1.0 );        // Vector from surface point to camera
    vec3 v = normalize(uCameraPosition-texture(gPosition,vTexcoord).rgb);                       // Vector from surface point to camera
